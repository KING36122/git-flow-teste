...
function cadastrarProduto($conexao, $nome, $imagem, $descricao, $idFornecedor){
	$query = mysqli_query($conexao, "INSERT INTO produto (NomeP, Imagem, Descricao, IdFornecedor) 
		VALUES 
...
function alterarProduto($conexao, $nome, $img, $desc, $idFor, $idproduto){
	$query = mysqli_query($conexao, "UPDATE produto SET NomeP='{$nome}', Imagem='{$img}', Descricao='{$desc}', IdFornecedor='{$idFor}' WHERE IdProduto = '{$idproduto}'");
	return $query;
}
...
function listarFornecedor($conexao){
	$query = mysqli_query($conexao, "SELECT * FROM fornecedor GROUP BY Nome");
	while($linha = mysqli_fetch_assoc($query)){
		$fornecedor[] = $linha;
	}
	return $fornecedor;
